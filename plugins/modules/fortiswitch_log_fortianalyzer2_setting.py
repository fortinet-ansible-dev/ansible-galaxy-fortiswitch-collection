...
DOCUMENTATION = '''
---
module: fortiswitch_log_fortianalyzer2_setting
...
def log_fortianalyzer2_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('log.fortianalyzer2', 'setting', filtered_data)
        current_data = fos.get('log.fortianalyzer2', 'setting', mkey=mkey)
...
def fortiswitch_log_fortianalyzer2(data, fos, check_mode):
    fos.do_member_operation('log.fortianalyzer2', 'setting')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["log_fortianalyzer2_setting"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortianalyzer2_setting"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "log_fortianalyzer2_setting")
        is_error, has_changed, result, diff = fortiswitch_log_fortianalyzer2(module.params, fos, module.check_mode)
    else:
