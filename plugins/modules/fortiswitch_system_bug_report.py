...
def filter_system_bug_report_data(json):
    option_list = ["auth", "mailto", "password", "server", "username", "username_smtp"]
...
def system_bug_report(data, fos, check_mode=False):
    state = data.get("state", None)

    system_bug_report_data = data["system_bug_report"]

    filtered_data = filter_system_bug_report_data(system_bug_report_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "bug-report", filtered_data)
        current_data = fos.get("system", "bug-report", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_system(data, fos, check_mode):
    ...
    if data["system_bug_report"]:
        resp = system_bug_report(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_bug_report"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_bug_report"
        )
