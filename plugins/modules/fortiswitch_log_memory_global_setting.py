...
DOCUMENTATION = '''
---
module: fortiswitch_log_memory_global_setting
short_description: Global settings for memory log in Fortinet's FortiSwitch
description:
...
options:
    ...
    log_memory_global_setting:
        ...
        suboptions:
            ...
            hourly_upload:
                description:
                    - Whether to enable scheduled upload hourly.
                type: str
                choices:
                    - 'enable'
                    - 'disable'
            max_size:
                description:
                    - Maximum size(byte) in memory buffer log.
                type: int
'''
...
def log_memory_global_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('log.memory', 'global-setting', filtered_data)
        current_data = fos.get('log.memory', 'global-setting', mkey=mkey)
...
def fortiswitch_log_memory(data, fos, check_mode):
    ...
    if data['log_memory_global_setting']:
        ...
        fos._module.fail_json(msg='missing task body: %s' % ('log_memory_global_setting'))
    if check_mode:
        return resp
...
versioned_schema = {
    ...
    "children": {
        "max_size": {
            ...
            "type": "integer",
            "name": "max-size",
            "help": "Maximum size(byte) in memory buffer log.",
            "category": "unitary"
        },
        "full_final_warning_threshold": {
            "v_range": [
        ...
        "hourly_upload": {
            ...
            "name": "hourly-upload",
            "help": "Whether to enable scheduled upload hourly.",
            "category": "unitary"
        }
    },
    "name": "global-setting",
    "help": "Global settings for memory log.",
    "category": "complex"
}
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["log_memory_global_setting"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_memory_global_setting"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "log_memory_global_setting")
        is_error, has_changed, result, diff = fortiswitch_log_memory(module.params, fos, module.check_mode)
    else:
