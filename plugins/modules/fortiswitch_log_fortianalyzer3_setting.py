...
DOCUMENTATION = '''
---
module: fortiswitch_log_fortianalyzer3_setting
...
options:
    ...
    log_fortianalyzer3_setting:
        ...
        suboptions:
            ...
            upload_option:
                description:
                    - Enable/disable logging to hard disk and then upload to FortiAnalyzer.
                type: str
                choices:
                    - 'store-and-upload'
                    - 'realtime'
...
def log_fortianalyzer3_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('log.fortianalyzer3', 'setting', filtered_data)
        current_data = fos.get('log.fortianalyzer3', 'setting', mkey=mkey)
...
def fortiswitch_log_fortianalyzer3(data, fos, check_mode):
    fos.do_member_operation('log.fortianalyzer3', 'setting')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['log_fortianalyzer3_setting']:
        resp = log_fortianalyzer3_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('log_fortianalyzer3_setting'))
    if check_mode:
        return resp
...
versioned_schema = {
    ...
    "children": {
        ...
        "upload_option": {
            ...
            "name": "upload-option",
            "help": "Enable/disable logging to hard disk and then upload to FortiAnalyzer.",
            "category": "unitary"
        },
        "buffer_max_send": {
            "v_range": [
        ...
        "upload_day": {
            ...
            "type": "string",
            "name": "upload-day",
            "help": "Days of week(month) to upload logs.",
            "category": "unitary"
        },
        "ips_archive": {
            "v_range": [
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["log_fortianalyzer3_setting"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortianalyzer3_setting"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "log_fortianalyzer3_setting")
        is_error, has_changed, result, diff = fortiswitch_log_fortianalyzer3(module.params, fos, module.check_mode)
    else:
