...
DOCUMENTATION = """
---
module: fortiswitch_system_flan_cloud
short_description: FortiLAN cloud manager configuration in Fortinet's FortiSwitch
description:
    - This module is able to configure a FortiSwitch device by allowing the
      user to set and modify system feature and flan_cloud category.
...
options:
    ...
    system_flan_cloud:
        ...
        suboptions:
            interval:
                description:
                    - Service name resolution time interval (3-300sec).
                type: int
            name:
                description:
                    - Fully qualified domain name or IP address of FortiLAN-cloud service.
                type: str
            port:
                description:
                    - Port Number.
...
def filter_system_flan_cloud_data(json):
    option_list = ["interval", "name", "port", "service_type", "status"]
...
def system_flan_cloud(data, fos, check_mode=False):
    state = data.get("state", None)

    system_flan_cloud_data = data["system_flan_cloud"]

    filtered_data = filter_system_flan_cloud_data(system_flan_cloud_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "flan-cloud", filtered_data)
        current_data = fos.get("system", "flan-cloud", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_system(data, fos, check_mode):
    fos.do_member_operation("system", "flan-cloud")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["system_flan_cloud"]:
        resp = system_flan_cloud(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_flan_cloud"))
    if check_mode:
        return resp
    return (
...
versioned_schema = {
    ...
    "children": {
        "status": {
            ...
            "name": "status",
            "help": "Enable/disable FortiLAN-cloud service.",
            "category": "unitary",
        },
        "interval": {
            "v_range": [["v7.0.3", ""]],
        ...
        "service_type": {
            "v_range": [["v7.4.2", ""]],
            "type": "string",
            "options": [{"value": "flan-cloud"}, {"value": "fortilink-https"}],
            "name": "service-type",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_flan_cloud"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_flan_cloud"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_flan_cloud"
        )
