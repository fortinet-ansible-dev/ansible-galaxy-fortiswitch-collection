...
def switch_storm_control(data, fos, check_mode=False):
    state = data.get("state", None)

    switch_storm_control_data = data["switch_storm_control"]

    filtered_data = filter_switch_storm_control_data(switch_storm_control_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("switch", "storm-control", filtered_data)
        current_data = fos.get("switch", "storm-control", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_switch(data, fos, check_mode):
    fos.do_member_operation("switch", "storm-control")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["switch_storm_control"]:
        resp = switch_storm_control(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "name": "storm-control",
    "help": "Configure excess switch traffic (storm control).",
    "category": "complex",
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_storm_control"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_storm_control"
        )
