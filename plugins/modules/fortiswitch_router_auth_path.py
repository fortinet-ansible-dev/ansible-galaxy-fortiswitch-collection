...
DOCUMENTATION = '''
---
module: fortiswitch_router_auth_path
short_description: Auth-based routing configuration in Fortinet's FortiSwitch
description:
...
options:
    ...
    router_auth_path:
        ...
        suboptions:
            device:
                description:
                    - Output interface. Source system.interface.name.
                type: str
            gateway:
                description:
                    - Gateway IP address.
...
def router_auth_path(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('router', 'auth-path', filtered_data)
        current_data = fos.get('router', 'auth-path', mkey=mkey)
...
def fortiswitch_router(data, fos, check_mode):
    ...
    fos.do_member_operation('router', 'auth-path')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["router_auth_path"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "router_auth_path")
