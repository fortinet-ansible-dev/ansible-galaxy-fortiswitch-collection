...
def system_fortianalyzer3(data, fos, check_mode=False):
    state = data.get("state", None)

    system_fortianalyzer3_data = data["system_fortianalyzer3"]

    filtered_data = filter_system_fortianalyzer3_data(system_fortianalyzer3_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "fortianalyzer3", filtered_data)
        current_data = fos.get("system", "fortianalyzer3", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_system(data, fos, check_mode):
    fos.do_member_operation("system", "fortianalyzer3")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["system_fortianalyzer3"]:
        resp = system_fortianalyzer3(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_fortianalyzer3"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_fortianalyzer3"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_fortianalyzer3"
        )
