...
DOCUMENTATION = """
---
module: fortiswitch_switch_ptp_policy
short_description: PTP policy configuration in Fortinet's FortiSwitch
description:
    - This module is able to configure a FortiSwitch device by allowing the
      user to set and modify switch_ptp feature and policy category.
...
def switch_ptp_policy(data, fos, check_mode=False):
    ...
    filtered_data = filter_switch_ptp_policy_data(switch_ptp_policy_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("switch.ptp", "policy", filtered_data)
        current_data = fos.get("switch.ptp", "policy", mkey=mkey)
        is_existed = (
            current_data
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("switch.ptp", "policy", mkey=filtered_data["name"])
    else:
...
def fortiswitch_switch_ptp(data, fos, check_mode):
    ...
    if data["switch_ptp_policy"]:
        resp = switch_ptp_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("switch_ptp_policy"))
    if check_mode:
        return resp
    return (
...
versioned_schema = {
    ...
    "children": {
        "status": {
            ...
            "name": "status",
            "help": "Enable/disable PTP policy.",
            "category": "unitary",
        },
        "name": {
            "v_range": [["v7.0.0", "v7.2.4"]],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_ptp_policy"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_ptp_policy"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_ptp_policy"
        )
