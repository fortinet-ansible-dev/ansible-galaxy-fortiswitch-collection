...
def system_sniffer_profile(data, fos, check_mode=False):
    ...
    if state == "present" or state is True:
        ...
        return fos.delete(
            "system", "sniffer-profile", mkey=filtered_data["profile-name"]
        )
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sniffer_profile"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_sniffer_profile"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_sniffer_profile"
        )
