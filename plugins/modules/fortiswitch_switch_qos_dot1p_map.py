...
def switch_qos_dot1p_map(data, fos):
...
def fortiswitch_switch_qos(data, fos):
    ...
    fos.do_member_operation('switch_qos_dot1p_map')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['switch_qos_dot1p_map']:
        resp = switch_qos_dot1p_map(data, fos)
    else:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        ...
        if is_secret_field(attribute_name):
            fields["switch_qos_dot1p_map"]['options'][attribute_name]['no_log'] = True
