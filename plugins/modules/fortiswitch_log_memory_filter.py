...
DOCUMENTATION = '''
---
module: fortiswitch_log_memory_filter
short_description: Filters for memory buffer in Fortinet's FortiSwitch
description:
...
def log_memory_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('log.memory', 'filter', filtered_data)
        current_data = fos.get('log.memory', 'filter', mkey=mkey)
