...
def filter_switch_security_feature_data(json):
    option_list = ['allow_mcast_sa', 'allow_sa_mac_all_zero', 'macsa_eq_macda',
                   'sip_eq_dip', 'tcp_flag', 'tcp_flag_FUP',
                   'tcp_flag_SF', 'tcp_hdr_partial', 'tcp_port_eq',
                   'udp_port_eq', 'v4_first_frag']
    dictionary = {}
...
def switch_security_feature(data, fos):
    switch_security_feature_data = data['switch_security_feature']
    filtered_data = underscore_to_hyphen(filter_switch_security_feature_data(switch_security_feature_data))
...
def fortiswitch_switch(data, fos):
    ...
    fos.do_member_operation('switch_security_feature')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['switch_security_feature']:
        resp = switch_security_feature(data, fos)
    else:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_security_feature"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_security_feature"]['options'][attribute_name]['required'] = True
        if is_secret_field(attribute_name):
            fields["switch_security_feature"]['options'][attribute_name]['no_log'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_security_feature")
