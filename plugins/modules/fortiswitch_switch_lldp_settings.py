    ...
    switch_lldp_settings:
        ...
        suboptions:
            ...
            management_interface:
                description:
                    - Primary management interface to be advertised. Source system.interface.name.
                type: str
            status:
                description:
                    - Enable/disable LLDP.
...
def filter_switch_lldp_settings_data(json):
    option_list = ['device_detection', 'fast_start_interval', 'management_interface',
                   'status', 'tx_hold', 'tx_interval']
    dictionary = {}
...
def switch_lldp_settings(data, fos):
    switch_lldp_settings_data = data['switch_lldp_settings']
    filtered_data = underscore_to_hyphen(filter_switch_lldp_settings_data(switch_lldp_settings_data))
...
def fortiswitch_switch_lldp(data, fos):
    ...
    fos.do_member_operation('switch_lldp_settings')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['switch_lldp_settings']:
        resp = switch_lldp_settings(data, fos)
    else:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        ...
        if is_secret_field(attribute_name):
            fields["switch_lldp_settings"]['options'][attribute_name]['no_log'] = True
