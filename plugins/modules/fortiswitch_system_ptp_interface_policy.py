...
def system_ptp_interface_policy(data, fos, check_mode=False):
    state = data.get("state", None)

    system_ptp_interface_policy_data = data["system_ptp_interface_policy"]

    filtered_data = filter_system_ptp_interface_policy_data(
        system_ptp_interface_policy_data
    )
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system.ptp", "interface-policy", filtered_data)
        current_data = fos.get("system.ptp", "interface-policy", mkey=mkey)
        is_existed = (
            current_data
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("system.ptp", "interface-policy", mkey=filtered_data["name"])
    else:
...
def fortiswitch_system_ptp(data, fos, check_mode):
    ...
    if data["system_ptp_interface_policy"]:
        resp = system_ptp_interface_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_ptp_interface_policy")
        )
    if check_mode:
        return resp
    return (
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_ptp_interface_policy"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_ptp_interface_policy"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_ptp_interface_policy"
        )
