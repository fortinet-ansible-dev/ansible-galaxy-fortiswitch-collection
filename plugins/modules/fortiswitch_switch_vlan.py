...
def switch_vlan(data, fos, check_mode=False):
    ...
    filtered_data = filter_switch_vlan_data(switch_vlan_data)
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("switch", "vlan", mkey=filtered_data["id"])
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_vlan"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_vlan"
        )
