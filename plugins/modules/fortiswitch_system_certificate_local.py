...
def system_certificate_local(data, fos, check_mode=False):
    ...
    filtered_data = filter_system_certificate_local_data(system_certificate_local_data)
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("system.certificate", "local", mkey=filtered_data["name"])
    else:
...
def fortiswitch_system_certificate(data, fos, check_mode):
    ...
    if data["system_certificate_local"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_certificate_local")
        )
    if check_mode:
        return resp
    return (
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_certificate_local"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_certificate_local"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_certificate_local"
        )
