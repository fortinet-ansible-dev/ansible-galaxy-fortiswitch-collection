...
def filter_switch_acl_policer_data(json):
    option_list = ['description', 'guaranteed_bandwidth', 'guaranteed_burst',
                   'id', 'maximum_burst', 'type']
    dictionary = {}
...
def switch_acl_policer(data, fos):

    state = data['state']

    switch_acl_policer_data = data['switch_acl_policer']
    filtered_data = underscore_to_hyphen(filter_switch_acl_policer_data(switch_acl_policer_data))
...
def fortiswitch_switch_acl(data, fos):
    ...
    fos.do_member_operation('switch_acl_policer')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['switch_acl_policer']:
        resp = switch_acl_policer(data, fos)
    else:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_acl_policer"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_acl_policer"]['options'][attribute_name]['required'] = True
        if is_secret_field(attribute_name):
            fields["switch_acl_policer"]['options'][attribute_name]['no_log'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_acl_policer")
