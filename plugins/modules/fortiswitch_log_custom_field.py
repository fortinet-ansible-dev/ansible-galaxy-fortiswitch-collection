...
def log_custom_field(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log", "custom-field", filtered_data)
        current_data = fos.get("log", "custom-field", mkey=mkey)
        is_existed = (
            current_data
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("log", "custom-field", mkey=filtered_data["id"])
    else:
...
def fortiswitch_log(data, fos, check_mode):
    fos.do_member_operation("log", "custom-field")
...
versioned_schema = {
    ...
    "v_range": [["v7.0.0", ""]],
    "name": "custom-field",
    "help": "Custom field configuation.",
    "mkey": "id",
    "category": "table",
}
