...
def filter_switch_network_monitor_settings_data(json):
    option_list = ["db_aging_interval", "status", "survey_mode", "survey_mode_interval"]
...
def switch_network_monitor_settings(data, fos, check_mode=False):
    state = data.get("state", None)

    switch_network_monitor_settings_data = data["switch_network_monitor_settings"]

    filtered_data = filter_switch_network_monitor_settings_data(
        switch_network_monitor_settings_data
    )
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("switch.network-monitor", "settings", filtered_data)
        current_data = fos.get("switch.network-monitor", "settings", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_switch_network_monitor(data, fos, check_mode):
    ...
    if data["switch_network_monitor_settings"]:
        resp = switch_network_monitor_settings(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_network_monitor_settings")
        )
    if check_mode:
        return resp
    return (
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_network_monitor_settings"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_network_monitor_settings"
        )
