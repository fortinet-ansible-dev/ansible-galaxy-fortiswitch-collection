...
def log_remote_setting(data, fos, check_mode=False):
    state = data.get("state", None)

    log_remote_setting_data = data["log_remote_setting"]

    filtered_data = filter_log_remote_setting_data(log_remote_setting_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.remote", "setting", filtered_data)
        current_data = fos.get("log.remote", "setting", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_log_remote(data, fos, check_mode):
    fos.do_member_operation("log.remote", "setting")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["log_remote_setting"]:
        resp = log_remote_setting(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_remote_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_remote_setting"
        )
        is_error, has_changed, result, diff = fortiswitch_log_remote(
