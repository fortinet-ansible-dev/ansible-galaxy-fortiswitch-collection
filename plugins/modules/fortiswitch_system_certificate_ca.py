...
DOCUMENTATION = """
---
module: fortiswitch_system_certificate_ca
short_description: CA certificate in Fortinet's FortiSwitch
description:
    - This module is able to configure a FortiSwitch device by allowing the
      user to set and modify system_certificate feature and ca category.
...
def system_certificate_ca(data, fos, check_mode=False):
    state = data.get("state", None)

    system_certificate_ca_data = data["system_certificate_ca"]

    filtered_data = filter_system_certificate_ca_data(system_certificate_ca_data)
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system.certificate", "ca", filtered_data)
        current_data = fos.get("system.certificate", "ca", mkey=mkey)
        is_existed = (
            current_data
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("system.certificate", "ca", mkey=filtered_data["name"])
    else:
...
def fortiswitch_system_certificate(data, fos, check_mode):
    fos.do_member_operation("system.certificate", "ca")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["system_certificate_ca"]:
        resp = system_certificate_ca(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_certificate_ca"))
    if check_mode:
        return resp
    return (
...
versioned_schema = {
    ...
    "children": {
        ...
        "auto_update_days_warning": {
            ...
            "help": "Days to send update before auto-update,0=disabled.",
            "category": "unitary",
        },
        "name": {
            "v_range": [["v7.0.0", ""]],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_certificate_ca"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_certificate_ca"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_certificate_ca"
        )
