...
DOCUMENTATION = '''
---
module: fortiswitch_log_syslogd2_filter
short_description: Filters for remote system server in Fortinet's FortiSwitch
description:
...
def log_syslogd2_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('log.syslogd2', 'filter', filtered_data)
        current_data = fos.get('log.syslogd2', 'filter', mkey=mkey)
...
def fortiswitch_log_syslogd2(data, fos, check_mode):
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["log_syslogd2_filter"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "log_syslogd2_filter")
        is_error, has_changed, result, diff = fortiswitch_log_syslogd2(module.params, fos, module.check_mode)
    else:
