...
def switch_mld_snooping_globals(data, fos, check_mode=False):
    state = data.get("state", None)

    switch_mld_snooping_globals_data = data["switch_mld_snooping_globals"]

    filtered_data = filter_switch_mld_snooping_globals_data(
        switch_mld_snooping_globals_data
    )
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("switch.mld-snooping", "globals", filtered_data)
        current_data = fos.get("switch.mld-snooping", "globals", mkey=mkey)
        is_existed = (
            current_data
...
def fortiswitch_switch_mld_snooping(data, fos, check_mode):
    fos.do_member_operation("switch.mld-snooping", "globals")
    current_cmdb_index = fos.monitor_get("/system/status")["cmdb-index"]
    if data["switch_mld_snooping_globals"]:
        resp = switch_mld_snooping_globals(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_mld_snooping_globals")
        )
    if check_mode:
        return resp
    return (
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_mld_snooping_globals"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_mld_snooping_globals"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_mld_snooping_globals"
        )
        is_error, has_changed, result, diff = fortiswitch_switch_mld_snooping(
