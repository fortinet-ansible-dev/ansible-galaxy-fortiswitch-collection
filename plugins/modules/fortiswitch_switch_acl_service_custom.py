...
def switch_acl_service_custom(data, fos, check_mode=False):
    ...
    filtered_data = filter_switch_acl_service_custom_data(
        switch_acl_service_custom_data
    )
    ...
    if state == "present" or state is True:
        ...
        return fos.delete("switch.acl.service", "custom", mkey=filtered_data["name"])
    else:
...
def fortiswitch_switch_acl_service(data, fos, check_mode):
    ...
    if data["switch_acl_service_custom"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_acl_service_custom")
        )
    if check_mode:
        return resp
    return (
...
versioned_schema = {
    ...
    "children": {
        ...
        "protocol": {
            "v_range": [["v7.0.0", ""]],
            "type": "string",
            "options": [{"value": "TCP/UDP/SCTP"}, {"value": "ICMP"}, {"value": "IP"}],
            "name": "protocol",
            "help": "Protocol type.",
            "category": "unitary",
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_acl_service_custom"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_acl_service_custom"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_acl_service_custom"
        )
        is_error, has_changed, result, diff = fortiswitch_switch_acl_service(
