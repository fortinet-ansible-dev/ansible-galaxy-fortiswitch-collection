...
DOCUMENTATION = '''
---
module: fortiswitch_system_automation_stitch
short_description: Automation stitches in Fortinet's FortiSwitch
description:
...
def filter_system_automation_stitch_data(json):
    option_list = ['action', 'name', 'status',
                   'trigger']
...
def system_automation_stitch(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('system', 'automation-stitch', filtered_data)
        current_data = fos.get('system', 'automation-stitch', mkey=mkey)
...
def fortiswitch_system(data, fos, check_mode):
    fos.do_member_operation('system', 'automation-stitch')
    current_cmdb_index = fos.monitor_get('/system/status')['cmdb-index']
    if data['system_automation_stitch']:
        resp = system_automation_stitch(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_automation_stitch'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_automation_stitch"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_automation_stitch"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_automation_stitch")
