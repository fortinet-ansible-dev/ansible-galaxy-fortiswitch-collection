...
class HttpApi(HttpApiBase):
    def __init__(self, connection):
        ...
        self._ansible_galaxy_version = "1.3.0"
        self._log = None
    ...
    def get_access_token(self):
        ...
        token = (
            self._conn.get_option("access_token")
            if "access_token" in self._conn._options
            else None
        )
    ...
    def login(self, username, password):
        ...
        dummy, result_data = self.send_request(
            url="/logincheck", data=data, method="POST"
        )
        self.log(
            "login with user: %s %s"
    ...
    def logout(self):
        ...
        self.log("logout")
        self.send_request(url="/api/v2/logout", method="POST")
    ...
    def update_system_version(self):
        ...
        """
        check_system_status = (
            self._conn.get_option("check_system_status")
            if "check_system_status" in self._conn._options
            else True
        )
